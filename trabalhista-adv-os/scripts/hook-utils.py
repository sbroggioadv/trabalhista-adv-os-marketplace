...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando trabalhista/ acima
- hook_state_load/save    → gerencia trabalhista/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "trabalhista"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/trabalhista/cowork-state.json."""
    start = start.resolve()
